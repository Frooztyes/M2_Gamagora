...
public class ChunkInfo
{
    ...
    readonly List<GameObject> dijkstraNodes;
    readonly List<GameObject> astarNodes;
    ...
    public void DeactivateChunk(Tilemap walls, Tilemap background)
    {
        ...
        foreach (GameObject node in dijkstraNodes)
        {
            if (!node.CompareTag("Start") || !node.CompareTag("End"))
                node.tag = "DeactivatedNode";
        }

        foreach (GameObject node in astarNodes)
        {
            node.tag = "DeactivatedNode";
        }
        IsActive = false;
    ...
    public ChunkInfo(
        ...
        List<GameObject> dijkstraNodes,
        List<GameObject> astarNodes,
